...
export const calculateBoykoResult = (
  ...
  Object.entries(SYMPTOM_QUESTIONS_MAP).forEach(([symptom, questionIds]) => {
    ...
    if (phase) {
      ...
      result[phase].symptoms[symptom] = {
        score: symptomScore,
        level: getSymptomLevel(symptomScore),
  ...
  (Object.keys(PHASE_SYMPTOMS_MAP) as Phase[]).forEach((phase) => {
    result[phase].level = getPhaseLevel(result[phase].total);
  });
...
const getSymptomLevel = (score: number): SymptomResult["level"] => {
  if (score >= 6) return "formed";
  if (score >= 4) return "forming";
  return "not_formed";
};

const getPhaseLevel = (score: number): PhaseResult['level'] => {
  if (score >= 16) return 'formed';
  if (score >= 10) return 'forming';
  return 'not_formed';
};
